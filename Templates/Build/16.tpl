...
echo "<h4 class=\"spacer\">Bejövő csapatok (".$units_incomming.")</h4>";
...
<h4 class="spacer">Csapatok a faluban és a saját oázisokban</h4>
	...
	<thead>
		<tr>
            ...
            <a href="spieler.php?uid=<?php echo $session->uid; ?>">Saját csapatok</a></td></tr></thead>
            <tbody class="units">
		...
		if(count($village->enforcetome) > 0) {
            foreach($village->enforcetome as $enforce) {
				...
				echo "<a href=\"spieler.php?uid=".$database->getVillageField($enforce['from'],"owner")."\">Egységek ".$database->getUserField($database->getVillageField($enforce['from'],"owner"),"username",0)."</a>";
